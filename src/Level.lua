...
function Level:init()
    ...
    table.insert(self.enemies, Alien(self.world, 'square', VIRTUAL_WIDTH - 80, VIRTUAL_HEIGHT - TILE_SIZE - ALIEN_SIZE / 2, 'Alien'))
...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        for k, alien in pairs(self.aliens) do
            ...
            if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) then
                ...
                if #self.enemies == 0 and #self.aliens == 0 then
                    gStateMachine:change('start')
                end

                if #self.aliens == 0 and #self.enemies ~= 0 then
                    self.canAbility = true
                    self.launchMarker = AlienLaunchMarker(self.world, self)
                end
                return
    ...
    local alien = self.launchMarker.alien
    if love.keyboard.wasPressed('space') and alien ~= nil and alien.ability and self.canAbility then
        alien.ability = false
        local velX, velY = alien.body:getLinearVelocity()
        local x, y = alien.body:getPosition()
        
        alienUp = Alien(self.world, 'round', x, y - 16, 'Player')
        alienDown = Alien(self.world, 'round', x, y + 16, 'Player')

        alienUp.body:setLinearVelocity(velX + 10, velY + 80)
        alienDown.body:setLinearVelocity(velX - 10, velY - 80)
        
        alienUp.fixture:setRestitution(0.4)
        alienDown.fixture:setRestitution(0.4)
        
        alienUp.body:setAngularDamping(1)
        alienDown.body:setAngularDamping(1)
        
        alienUp.body:setAngularVelocity(1.5)
        alienDown.body:setAngularVelocity(0.5)
